...
export interface WallFoundation {
  ...
  // Add a flag to track whether we've tried to move villagers off this tile
...
export class WallManager {
    ...
    public updateFoundationBuilding(delta: number): void {
      ...
      foundationsToUpdate.forEach(foundation => {
        // Check if there are villagers on the foundation tile that need to be cleared
        const villagersOnTile = this.getVillagersOnTile(foundation.x, foundation.y);
        
        // If there are villagers on the tile and we haven't cleared them yet
        if (villagersOnTile.length > 0 && !foundation.villagersCleared) {
          console.log(`Found ${villagersOnTile.length} villagers on foundation at (${foundation.x}, ${foundation.y})`);
          
          // Try to move each villager away from the foundation
          let allVillagersCleared = true;
          
          villagersOnTile.forEach(villager => {
            // Skip villagers that are assigned to build this foundation
            if (
              ...
              console.log('Skipping assigned builder villager');
              return;
            }
            
            const moved = this.forceVillagerMove(villager);
            if (!moved) {
              console.warn('Failed to move villager away from foundation');
              allVillagersCleared = false;
            }
          });
          
          // Mark foundation as cleared if all villagers were moved successfully
          if (allVillagersCleared) {
            foundation.villagersCleared = true;
          } else {
            // Don't proceed with building if villagers couldn't be cleared
            return;
          }
        }
        
        // Continue with existing building logic only if the tile is clear or villagers were successfully moved
    ...
    public createWallFoundation(x: number, y: number): WallFoundation | null {
      ...
      const foundation: WallFoundation = {
        ...
        status: 'foundation',
        occupiedPositions: [],
        villagersCleared: false // Initialize the new flag
      };
